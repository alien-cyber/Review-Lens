         ...
         let canTranslate='readily';
          if (canTranslate !== 'no') {
            if (canTranslate !== "after-download") {
              ...
              translator_input = await ai.translator.create({sourceLanguage:lang,targetLanguage:'en'});
              translator_output = await ai.translator.create({sourceLanguage:'en',targetLanguage:lang});
            
            }
             else {
            translator_input = null;
              translator_output = null;
              translator = await ai.translator.create({sourceLanguage:lang,targetLanguage:'en'});
...
async function botResponse() {
  ...
  if (relevantSentences.length === 0) {
    appendMessage_bot('information is still fetching','user','title','info');
    if(alert){
...
buttons.forEach(button => {
  button.addEventListener('click', async () => {
      // Skip the button with id 'sendMessageButton'
      if (button.id === 'sendMessageButton') return;
      ...
      await changeLanguage_input(lang);
  });
});
